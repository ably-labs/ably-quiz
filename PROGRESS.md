...
- [x] S5.2 design polish pass — landing front-door + /create; hero image + identity system (team marks); land-in-/host + share panel (dropped the quiz-ready page); agent availability (grey-out + exclude); overt progress bar; Advanced-options disclosure. See S5.2 note below.
- [x] S5.3 close the agent-reasoning wire-leak — status-only agent channels; quips host-mediated, released at reveal. See S5.3 note below.
- [ ] S5.3 full dry run (~10 humans + roster + 300 synthetic) + tuning
- [x] S5.4 README + docs/RUNBOOK.md
- [ ] **GATE: quiz-day definition of done (see brief)**

**S5.3 note — wire-leak closed (2026-07-14):** closed the mid-question agent-reasoning wire-leak flagged as the S5.2 known limitation. **Invariant:** while a question is asking/locked, the player-readable `quiz-agent:{id}:{slug}` channels carry STATUS only (thinking / answered / error) — no reasoning text, no quip. Each agent's one-liner now rides the `answerMessage` on the host-subscribe-only answers fan-in (players hold `publish` but NOT `subscribe` there — verified in `auth.ts`), so it can't leak to a player; the host (`useHostQuiz`) captures quips per idx off the fan-in and re-releases the batch at reveal via a new `agent-quips` message on the main channel. `/screen` reads that (`useQuizState.agentQuips`, live + history-seeded for late/reloaded joins) and `AgentQuipWall` now sources quips from it, gated to the revealed idx (errors still come from the status hook). Protocol: `answerMessageSchema` gained optional `quip`; added `agentQuipsSchema`/`parseAgentQuips`. `/api/agent-turn` no longer publishes any think-aloud text (dropped the throttled `onThinking` emit and the settle's `text`/`quip`); it emits `answered` with empty text and puts the quip on the fan-in answer. Kept entirely in the web layer — the core quizmaster engine never sees quips. Core untouched (`answerQuestion`/scoring unchanged); 6 new protocol tests. NB the persistent co-hosted runner (`pnpm agents:start`) is a separate dev path that still streams its think-aloud over AIT; the default on-demand path is now clean.

**S5.4 note (2026-07-14):** rewrote the stale root `README.md` (front door for open-sourcing) and added `docs/RUNBOOK.md` (quiz-day guide: before-the-event checklist, running a live quiz, failure playbook, scale, known limitations). Every command/env var/route/channel verified against the code, not the brief — one correction surfaced: **`HOST_KEY` is gone** (removed in the create-flow review; only lingers in BRIEF.md + the stale `spikes/auth-e2e/README.md`), so a humans-only quiz needs just `ABLY_API_KEY`. No `LICENSE` file yet (S6.5) — README notes MIT-in-the-open-source-pass rather than claiming one. Hero banner embeds the committed `apps/web/public/hero.webp`; real gameplay screenshots deferred to S6.5.
...
- [x] S6.3 MCP-powered study() — `ably-mcp` strategy: agents research Ably knowledge through the read-only MCP into synthesized facts cribs. Auth is **interactive OAuth** (DCR + PKCE + Okta, loopback redirect — no token in env). Ran it: all 8 cribs regenerated against the live MCP with grounded Ably facts (~500 words each), narration stripped, scanned public-safe. See S6.3 note below.
- [x] S6.4 PR-your-own-agent — agent-as-a-module contract + **CI harness landed** (see notes)
- [~] S6.5 open-source pass (Ably Labs) — **published to https://github.com/ably-labs/ably-quiz, CI green.** MIT LICENSE (© Ably Real-time Ltd), visual README (hero + badges + screenshots), local-machine paths scrubbed from history (`git filter-repo`), secrets swept clean, repo topics set. Remaining: Ably logo + gameplay/podium screenshots (Matt to add), then Vercel deploy.
- [x] S6.6 **agent conversation viewer** — full per-question transcripts (prompt, reasoning, tool calls, timing, answer) captured in the turn, carried on the host-subscribe-only fan-in, released at reveal (same wire-safe path as quips); end-of-quiz click-an-agent modal reads main-channel history. No DB. Verified in-browser 2026-07-15.
- [x] S6.7 **client-side MCP grounding loop** — replaced Anthropic's `mcp_servers` connector (~5s/call, 300s stalls) with a direct Streamable-HTTP tool loop (`mcp-client.ts`): real flattened tools + schemas to the model, direct execution (~0.1-0.3s/call). Requires a **full-mode token** — `?mode=full` is read at OAuth AUTHORIZE time and baked into the token (verified against ably-core-mcp source; 274 flattened tools), so the app + study CLI request it at sign-in. Live-verified: Jira question → 1 direct call → correct grounded answer.
- [x] S6.8 per-tool-call latency (`ms`) captured in transcripts and shown in the viewer.
- [x] S6.11 **grounded-turn reliability** — diagnosed the live "newest employee" failure (every agent whiffed): tools were fine (0.1s, data present); the failures were boolean queries that the MCP search matches literally (0 hits — plain phrases hit), one agent hallucinating Slack channel IDs, one agent skipping lookups from time-budget fear, and search-retry spirals burning the deadline with no answer. Fixes: HOW TO SEARCH / NEVER INVENT PARAMS / round-cost calibration in the grounding prompt; a hard `toolBudgetMs` (deadline − 5s, predictive by last round's duration) with a guaranteed forced answer round (roles-alternation + tool_choice none + 400-token cap); tool feedback trimmed 8K→4K. Bench on the failed question: 5-6/8 correct per run with genuine lookups vs 1 lucky guess before. Flagged: Slack results can include DM content (host token) — prompt forbids using it; server-side exclusion suggested for ably-os.
- [x] S6.10 **all-provider grounding** — the client-side MCP tool loop now runs for EVERY provider, not just Anthropic: Anthropic agents keep the direct-API loop, everyone else drives the same loop through the AI gateway's OpenAI-compatible function calling (`streamGatewayGrounded`). Empirically verified every roster model emits correct tool calls via the gateway, then live-benched all 8 agents grounded on a Confluence question: **8/8 correct with real tool calls** (4.2–12.6s). Added a forced final no-tools turn for tool-happy models that burn the loop without answering (live-observed with grok-4.20) — the gathered context becomes an answer instead of falling back ungrounded. Route requires the transport's key per provider.
- [x] S6.9 **shared MCP sessions + calibrated grounding** — one initialized session per server×token (promise-cached, concurrent turns dedupe, 404 → invalidate+retry, 30min idle prune), host pre-warms via `/api/agent-prewarm` at quiz start, batched tool_use blocks run concurrently. Warm turns: no-tool ~2.5-3s (was ~7.6s), 1-tool ~5.7s (was ~9.7s), 2-tool batched ~7.7s — benchmarked 6/6 correct 2026-07-16. Prompt: ACCURACY BEATS SPEED — verify company data when uncertain, batch parallel lookups, guess only when tools come up empty. **Safety:** `ABLY_MCP_TOOLS` allowlist is now MANDATORY for grounding (full mode exposes write tools; server enforces no allowlist) — enforced in both the route and the runner.

**S6.3 note (2026-07-14):** the `study()` scaffolding existed from S4.3 (deterministic `ably-docs` doc-scrape); S6.3 adds the MCP-powered `ably-mcp` strategy — the model trawls Ably knowledge via the read-only MCP MCP connector and synthesizes a facts crib. Strategy is pure/testable (injected `research` hook). **Auth:** Matt didn't want a token in env, so the CLI runs the same OAuth the app does — DCR + PKCE (S256) against the Worker, adapted for a CLI via an RFC 8252 **loopback redirect** (`mcp-oauth.ts`): prints an authorize link, user signs in through Okta, a localhost server catches the callback, token lives in memory for the run only. `ABLY_MCP_AUTH` remains an optional CI override; no TTY / declined auth ⇒ skip gracefully. Ran it live (Matt's Okta) — all 8 cribs regenerated with grounded facts. One fix landed: the grounded model streams tool-use narration before the crib, so a `CRIB_SENTINEL` + strip removes it. Guardrail in the study prompt keeps cribs public-safe (repo is open-source-bound); output scanned clean. Roster all on `study: ably-mcp`.

**Agent-as-a-module note (2026-07-14, advances S6.4):** Matt asked for an architecture where every builder can design their own agent logic, even if they reuse shared pieces. Each `agents/<slug>/agent.ts` may now export `study` and/or `answer` hooks (`AgentModule` in `agent-module.ts`), reusing shared blocks (`ablyMcpStudy`, `answerQuestion`) or replacing them. Resolution: study → agent.ts, else the named JSON strategy, else none; answer → agent.ts, else the default core. The CLIs load agent.ts dynamically (`agent-loader.ts`, tsx). The bundled web app can't, so `pnpm agents:build` codegens a **static index** (`apps/web/lib/agent-modules.generated.ts`) that the on-demand `/api/agent-turn` route imports — so a custom `answer()` runs in the live turn (Matt chose this over local-only). To let agent.ts import the shared package, `agents/` became a single workspace package (folders stay a path-loaded registry). `matt-opus/agent.ts` is a worked example; `agents/README.md` documents the contract. Verified the web route bundles the cross-boundary chain and serves.

**CI harness note (2026-07-14, completes S6.4):** a contributor's agent PR is now validated automatically. New `pnpm agent:validate` (`validate-cli.ts`) runs `loadRegistry` (zod contract + slug/folder match + crib readable on every `agent.json`) and imports every `agent.ts` — a broken behaviour module fails too. To surface import failures (which `loadAgentModules` swallows by design so one bad PR can't fell the live roster), it gained an optional `onError` collector the validator passes; the host still logs-and-skips. A new `agents` CI job (parallel to `quality`, same pnpm/Node-24 setup) runs `pnpm agent:validate` then a module-index drift check (`pnpm agents:build` + `git diff --exit-code apps/web/lib/agent-modules.generated.ts`, with an `::error::` telling the contributor to rebuild + commit) so adding/removing an `agent.ts` without regenerating fails. `pnpm agents:check` reproduces the drift check locally. The model-backed `agent:test` is deliberately NOT in CI (needs a provider key); schema validation is the CI-safe check. Verified: `agent:validate` exits 0 for all 8 agents, the drift check is clean on the current tree, and both fail as expected on a stale index / a broken agent.ts. `agents/README.md` + `CONTRIBUTING.md` document the pre-PR steps.

**S5.2 note (2026-07-14):** polish pass from Matt's manual-test feedback. (1) Real front door: `/` is a landing (hero + Host/Join); the create grid moved to `/create`. (2) Hero image (`public/hero.webp`, a brain arm-wrestling a chip) on landing + `/screen` lobby + `/play` wait; derived `team-carbon`/`team-silicon` marks give identity continuity in rosters + the tug-of-war (`<TeamMark>`) without dragging the scene onto every screen. (3) Create → land in `/host`, which grew a share panel (QR + join link + "Open big screen"); the standalone quiz-ready page + its confusing "Open screen / Open host controls" pair are gone. (4) Agent availability: preflight-failed agents are greyed "unavailable", excluded from the expected-answerer count, and skipped when firing turns (fixes the Grok-503 stall); grounding/health banners now show only in the lobby or on a genuine problem. (5) Overt segmented progress bar on `/host` + `/screen`. (6) Streak + auto-reveal tucked under an Advanced disclosure on `/create`. Follow-ups: `/screen` roster greys unavailable agents only if the host broadcasts availability (currently host-only); a square hero variant would be needed for player-phone/OG use; the raster hero was generated externally (Matt) — I can't generate images, so this is a hand-off point for future art. **Final pass (2026-07-14):** end screens reordered to result-first (podium/mini-podium), a pundit "lower-third" `CommentaryCard` replacing the orange verdict billboard with the verdict trimmed to ~80 words, tighter readable face-crop team marks re-cut from the hero with top-right Carbon-vs-Silicon identity now on host + screen (not just play), the MCP auth banner collapsing from tick to a compact chip, a "not grounded" confirm on Start when agents are declared, a core fix healing stale raw-id scoreboard names when presence arrives late, and shared-screen copy. **Screen reasoning-leak fix (2026-07-14, live-test finding):** agent reasoning is no longer shown on `/screen` during open questions — the streamed think-aloud rendered mid-question and leaked the answer to anyone reading the shared projector (e.g. "Seven continents: Africa, Antarctica…" while the question was still open). Split by phase: a compact status-only `<AgentStatusStrip>` (emoji + name + thinking…/✓/⚠️/ready chips) under the tally bars while asking/locked, and quip-only `<AgentQuipWall>` cards at reveal; the old `<AgentThinkingWall>` (which rendered the full reasoning text) is deleted. **Known limitation (CLOSED in S5.3 — see below):** the think-aloud text still streams on the `quiz-agent:{id}:{slug}` channels mid-question, so a devtools-savvy player could subscribe and peek at the wire; the clean fix (host-held quips published only at reveal, or a dedicated screen role) is deferred to S5.3.
